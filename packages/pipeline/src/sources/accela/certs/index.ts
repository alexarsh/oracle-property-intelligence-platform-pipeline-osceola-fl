/**
 * Entrust OV TLS Issuing RSA CA 2 (cross-signed by Sectigo Public Server
 * Authentication Root R46). permits.osceola.org serves its leaf certificate
 * WITHOUT this intermediate, so Node cannot build a chain to a bundled root.
 *
 * The bytes are identical to `entrust-ov-tls-issuing-rsa-ca-2.pem` in this
 * directory (a test enforces that); the string form exists because `tsc`
 * does not copy `.pem` files into `dist/`.
 *
 * @module sources/accela/certs
 */
export const ENTRUST_OV_TLS_ISSUING_RSA_CA_2_PEM = `-----BEGIN CERTIFICATE-----
