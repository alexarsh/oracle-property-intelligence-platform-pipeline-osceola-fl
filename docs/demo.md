...
Follows the assignment's Demo Transcript line by line. Values below are for the
latest published run at the time of writing; `artifacts/run-history.json` and the
Explorer's run summary are the source of truth if a newer run has been published.

**Before recording:** open each Explorer page and run one leads search and one
agent prompt once. The first request after idle can take up to a minute while
the MCP cold-starts and the gateway warms the new Parquet; every later request
is sub-second.
...
| What                  | Where                                                                                                                                        |
| --------------------- | -------------------------------------------------------------------------------------------------------------------------------------------- |
| Explorer UI           | `https://osceola-explorer.vercel.app`                                                                                                        |
| Elephant MCP endpoint | `https://osceola-mcp.vercel.app/mcp` (health: `https://osceola-mcp.vercel.app/health`)                                                       |
| Pull request          | https://github.com/prismteam-ai/oracle-property-intelligence-platform-pipeline-osceola-fl/pull/1                                             |
| Latest run manifest   | `artifacts/runs/2026-09-07T12-33-03Z-incremental/manifest.json`                                                                              |
| Run root CID          | `bafybeiduo5phbfpr3j76ssk45xl4smvcwrjfafvmtwl3qxlc6atduevbvm` (previous run: `bafybeieyru5hga3ypyatj2r342hejqifid65bw3rrff5ini2bfk4qifhwy`)  |
| IPNS name (pointer)   | `k51qzi5uqu5dj6np6f9gfvamtn0z1a4kcnbn1jpaba5im6i1msabllgebus21p` → resolves to `bafybeiduo5phbfpr3j76ssk45xl4smvcwrjfafvmtwl3qxlc6atduevbvm` |

## 1. "I am opening the pipeline run summary"

Open `https://osceola-explorer.vercel.app/`.
...
Open `https://osceola-explorer.vercel.app/sources`.
...
Open `https://osceola-explorer.vercel.app/query`.
...
Open `https://osceola-explorer.vercel.app/manifest`.
...
```bash
curl -sL https://ipfs.io/ipfs/bafybeier3a35cxykf4uuq3dnmmnjqcdo6b3p3aju2hu2ebcpygsnytuu6i | shasum -a 256
curl -sL https://dweb.link/ipfs/bafybeier3a35cxykf4uuq3dnmmnjqcdo6b3p3aju2hu2ebcpygsnytuu6i | shasum -a 256
# both equal the manifest digest sha256:b027da8a062b80373f037d1d36f00a9c4dfd0f85facbdbc7d729fa279bfdd9f8
```
...
`bafybeiduo5phbfpr3j76ssk45xl4smvcwrjfafvmtwl3qxlc6atduevbvm`; the previous run's root `bafybeieyru5hga3ypyatj2r342hejqifid65bw3rrff5ini2bfk4qifhwy` still resolves
...
Open `https://osceola-explorer.vercel.app/leads`, pick **Kissimmee**, radius 5 mi, threshold 15.
...
Open `https://osceola-explorer.vercel.app/agent` and click the two prepared prompts:
...
Open `https://osceola-explorer.vercel.app/mcp`: the endpoint, live `tools/list`, and the copy-paste
