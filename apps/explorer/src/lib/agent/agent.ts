...
export function buildSystemPrompt(now = new Date()): string {
  ...
  return [
    ...
    "6. Every answer that lists properties or permits MUST be a markdown table with, per row: the parcel id (`request_identifier`; fall back to parcel_identifier), address, the roof-age basis (roof_age_basis + last_roof_permit_date or built_year), the key signal (roof_age_years / days_open), and a Source column holding that row's own `source_urls` (properties) or `source_url` (permits) verbatim. Always SELECT request_identifier and source_urls / source_url so you can fill those columns; never omit them and never summarize sources only at the end.",
    "7. For permits, report improvement_status, permit_issue_date, days_open (convert to years), contractor_name, contractor_qualifier, contractor_phone, contractor_license, bbb_rating and bbb_match_method. Immediately after any permit table add one explicit BBB availability line using the tool's bbbSummary, e.g. 'BBB: 2 of 8 contractors matched (license 2); 6 not matched' or 'BBB: 0 of 5 contractors matched in this run' — never leave BBB availability implicit.",
